...
Deze publicatie strategie is vooral voor organisaties en gebruikers van belang die een beperkt aantal services willen publiceren (een OGC catalogue service is dan snel een te zware oplossing gezien het aantal te publiceren services). Dit is een eenvoudige manier voor metadata publicatie.
...
Beginnend van scratch lijkt een metadata editor heel meest eenvoudig, maar is de meest kostbare manier om een metadata document te maken. De gebruiker voert namelijk alle vereiste metadata elementen handmatig in en publiceert daarna het document als een valide CSW2 AP ISO document (ISO 19115/19139 en ISO 19119 encoding).
...
3. Bestaande metadata uit een catalog service harvesten
...
### Metadata uit een catalog service harvesten

Voor organisaties die zelf veel metadata hebben is het ook mogelijk zelf een catalog service op te zetten. Deze catalog service bevat de metadata en kan  door andere registers automatisch met een bepaalde interval geharvest worden.
...
2. Bestaande metadata vanuit een catalogue service harvesten
